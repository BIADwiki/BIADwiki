...
get.related.data <- function(table.name, primary.value, fnc, user, password){
    ...
    hl.data <- all.data[[table.name]][[primary.value]]
	# relative level 1 data
    rec.tree <- function(hl.data,keys,fnc,user,password){
        rel.names <- names(hl.data)
        rel.names <- rel.names[rel.1.names!='data']	
        for(rel.name in rel.names){
            x.data <-hl.data[rel.name]
            x.sub <- wrapper(keys, x.data, fnc, user, password)
            if(!is.null(x.sub))hl.data[rel.names] <- Map(c, x.data,x.sub)
            hl.data[[rel.name]] <- rec.tree(hl.data[rel.names],keys,fnc,user,password)
        }
    }
